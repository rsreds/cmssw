...
class SoATracksProducer : public edm::global::EDProducer<> {
public:
  explicit SoATracksProducer(const edm::ParameterSet&);
  ~SoATracksProducer();
...
// Constructor
SoATracksProducer::SoATracksProducer(const edm::ParameterSet& iConfig): generalTracksToken_(consumes<reco::TrackCollection>(iConfig.getParameter<edm::InputTag>("generalTracks"))) {
...
// Destructor
SoATracksProducer::~SoATracksProducer() {}

// Macro to label the label the module
DEFINE_FWK_MODULE(SoATracksProducer);

// Method to produce
void SoATracksProducer::produce(edm::StreamID iID, edm::Event& event, const edm::EventSetup& iSetup) const {
  edm::Handle<reco::TrackCollection> generalTracks_H;
  event.getByToken(generalTracksToken_, generalTracks_H);

  // get the size of the TrackingParticleCollection
  std::size_t elems = generalTracks_H->size();
  ...
  for (size_t i = 0; i < elems; i++) {
    ...
    double x = generalTracks_H->at(i).innerPosition().x();
    double y = generalTracks_H->at(i).innerPosition().y();
    double z = generalTracks_H->at(i).innerPosition().z();
    ...
    SoAviewSDF.x()[i] = static_cast<short>(x);
    SoAviewSDF.y()[i] = y;
    ...
    (*AoSProductSDF)[i].x = static_cast<short>(x);
    (*AoSProductSDF)[i].y = y;
    ...
    SimpleSoAProductSDF->x.push_back(static_cast<short>(x));
