...
class SoARandomProducer : public edm::global::EDProducer<> {
public:
  explicit SoARandomProducer(const edm::ParameterSet&);
  ~SoARandomProducer();
...
private:
    ...
    mutable std::normal_distribution<double> dist{0.0, 1.0}; // mean = 0, stddev = 1
};

// Constructor
SoARandomProducer::SoARandomProducer(const edm::ParameterSet& iConfig) {
...
// Destructor
SoARandomProducer::~SoARandomProducer() {}

// Macro to label the label the module
DEFINE_FWK_MODULE(SoARandomProducer);

// Method to produce
void SoARandomProducer::produce(edm::StreamID iID, edm::Event& event, const edm::EventSetup& iSetup) const {
  size_t elems = 100000;
